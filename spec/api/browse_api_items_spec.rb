...
describe 'Ebay browse api' do
  before(:all) do
    ...
    context 'when find certain group of items by group id', :test do
      before(:all) do
        @response = ItemApi.new(@token).get_group_of_items(110_552_751_758)
      end
    ...
    context 'when API codes' do
      it 'gets response' do
        response = ItemsGroupIdsApi.new(@token).get_items(385_348_059_626)
        expect(response.code).to eq 200
      end

      it 'checks response on the request' do
        response = ItemsGroupIdsApi.new(@token).get_items(234_743_947_831)
        expect(response.code).to eq 200
      end
    end
  end
